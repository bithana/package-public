...
test('Can parse csv file', async () => {
  const data = await csv.parse_csv_file(DATA1_PATH)
  await expect(data).toHaveLength(3)
})
...
a3|b3|c3
`
  const data = await csv.parse_csv_string(csv_str, { delimiter: '|' })
  await expect(data).toHaveLength(3)
})

it('should parse csv file with different delimiter', async () => {
  const data = await csv.parse_csv_file(DATA2_PATH, { delimiter: '|' })
  await expect(data).toHaveLength(3)
})
